...
const TokenBalance = ({ name, walletAddress}) => {
  ...
  async function fetchTokenBalance() {
    ...
    console.log(bal);
    const fBal = ethers.untils.formatUnits(bal.toString(), 18);
    setBalance(fBal.toString());
  }

  async function fetchTokenAddress() {
    const address = await getTokenBalance(name);
    setTokenAddress(address);
  }

  return(
    <div className="flex mx-2 border-[1px] rounded-l rounded-r-lg border-[#7765F3]">
      <div className="flex items-center bg-zinc-900 text-zinc-300 w-fit p2 px-3 rounded-l-lg">
